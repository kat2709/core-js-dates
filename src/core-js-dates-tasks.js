...
function dateToTimestamp(date) {
  const xDay = new Date('01 Jan 1970 00:00:00 UTC');
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return Math.floor(
    (Date.parse(dateEnd) - Date.parse(dateStart)) / 86400000 + 1
  );
}
...
function isDateInPeriod(date, period) {
  ...
  return periodEnd >= Date.parse(date) && Date.parse(date) >= periodStart;
}
...
function formatDate(date) {
  ...
  return `${curDate.getMonth() + 1}/${curDate.getUTCDate()}/${curDate.getFullYear()}, ${hours}:${String(curDate.getMinutes()).padStart(2, '0')}:${String(curDate.getSeconds()).padStart(2, '0')} ${partOfDay}`;
}
